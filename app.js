            ...
            if (aStatError) {
                ...
                if (aStats.isFile() && aTask.path.match(/\.[uU][rR][lL]$/)) {
                    ...
                    var lNewFileName = aTask.path.match(/(.*)\.[uU][rR][lL]$/)[1] + '.html',
                        lFileContent = fs.readFileSync(aTask.path, 'utf8'),
                        lFileContentMatch = lFileContent.match(/=(.*)/),
                        lNewFileContentTemplate = '<HTML><HEAD><META HTTP-EQUIV="Refresh" CONTENT="0; URL=XXX"></HEAD><BODY></BODY>',
                        lNewFileContent = '';
